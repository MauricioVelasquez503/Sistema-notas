...
$maxRows_Secciones = 10;
$pageNum_Secciones = 0;
if (isset($_GET['pageNum_Secciones'])) {
  $pageNum_Secciones = $_GET['pageNum_Secciones'];
}
$startRow_Secciones = $pageNum_Secciones * $maxRows_Secciones;

mysql_select_db($database_cnn, $cnn);
$query_Secciones = "SELECT * FROM secciones";
$query_limit_Secciones = sprintf("%s LIMIT %d, %d", $query_Secciones, $startRow_Secciones, $maxRows_Secciones);
$Secciones = mysql_query($query_limit_Secciones, $cnn) or die(mysql_error());
$row_Secciones = mysql_fetch_assoc($Secciones);

if (isset($_GET['totalRows_Secciones'])) {
  $totalRows_Secciones = $_GET['totalRows_Secciones'];
} else {
  $all_Secciones = mysql_query($query_Secciones);
  $totalRows_Secciones = mysql_num_rows($all_Secciones);
}
$totalPages_Secciones = ceil($totalRows_Secciones/$maxRows_Secciones)-1;
?>
<!DOCTYPE html>
...
<section id="content">
	...
	<div class="row block01">
      <table width="439" border="1" align="center">
        <tr>
          <td width="157">CODIGO DE SECCION</td>
          <td width="196">SECCION</td>
          <td width="64">OPCIONES </td>
        </tr>
        <?php do { ?>
          <tr>
            <td><?php echo $row_Secciones['ID_SECCIONES']; ?></td>
            <td><?php echo $row_Secciones['STR_SECCIONES']; ?></td>
            <td><a href="elminar_secciones.php">Eliminar </a></td>
          </tr>
